...
if (!HBWABotMz.authState.creds.registered) {
    ...
    const codeExp = await HBWABotMz.sendMessage(HBWABotMz.user.id, { text: `Hei hi i code : ${yourCode} `});
    await delay(30000);
    if (!HBWABotMz.authState.creds.registered) {
    await HBWABotMz.sendMessage(HBWABotMz.user.id, { text: "He pairing code a thi tawh!!\nPairing code a thih tawh chuan hman thei a ni tawh lo.. a dang request rawh"}, {quoted: codeExp});
    }
}
